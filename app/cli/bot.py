...
async def _bot_settings():
    ...
    async with AsyncSessionFactory() as session:
        ...
        for setting in settings:
            value_display = (
                setting.value[:50] + "..." if len(setting.value) > 50 else setting.value
            )
            table.add_row(setting.key, value_display, setting.description or "-")
...
async def _get_setting(key: str):
    ...
    async with AsyncSessionFactory() as session:
        ...
        click.echo(
            f"Обновлено: {setting.updated_at.strftime('%Y-%m-%d %H:%M:%S') if setting.updated_at else '-'}"
        )
