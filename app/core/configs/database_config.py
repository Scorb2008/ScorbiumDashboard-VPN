...
class _DatabaseConfig(BaseSettings):
    ...
    db_engine: Literal["postgresql"] = Field(default=..., validation_alias="DB_ENGINE")
    ...
    def validate_db_config(self) -> "_DatabaseConfig":
        ...
        if self.db_engine == "postgresql":
            ...
            log.info(
                f"✅ PostgreSQL configuration validated for database '{self.db_name}'"
            )
    ...
    def validate_db_engine(cls, value: str) -> str:
        ...
        if value not in supported_engines:
            log.error(
                f"❌ Engine '{value}' is not supported! Supported engines: {supported_engines}"
            )
    ...
    def validate_db_name(cls, value: str) -> str:
        ...
        if not re.match(r"^[a-z][a-z0-9_]{0,62}$", value):
            log.error(
                f"❌ Database name '{value}' contains invalid characters. Use only lowercase letters, numbers, and underscores"
            )
    ...
    def validate_db_user(cls, value: str) -> str:
        ...
        if not re.match(r"^[a-zA-Z_][a-zA-Z0-9_]{0,62}$", value):
            log.error(
                f"❌ Database user '{value}' contains invalid characters. Use only letters, numbers, and underscores"
            )
