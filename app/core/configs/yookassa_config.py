...
class _YookassaConfig(BaseSettings):
    ...
    yookassa_shop_id: Optional[int] = Field(
        default=None, ge=10000, le=999999999, validation_alias="YOOKASSA_SHOP_ID"
    )

    yookassa_secret_key: Optional[SecretStr] = Field(
        default=None, validation_alias="YOOKASSA_SECRET_KEY"
    )
    ...
    def validate_yookassa_shop_id(cls, value: Optional[int]) -> Optional[int]:
        ...
        if not (5 <= len(shop_id_str) <= 8):
            raise YookassaValueError(
                f"Shop ID can be from 5 to 8 numbers, values: {len(shop_id_str)}"
            )
    ...
    @classmethod
    def validate_yookassa_secret_key(
